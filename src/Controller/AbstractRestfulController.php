...
abstract class AbstractRestfulController extends LaminasAbstractRestfulController implements
	...
	protected function jsonEncodeColumns(array $data): array {
		foreach ($this->_processFields as $field => $type) if ($type == self::CONTENT_TYPE_JSON) if (array_key_exists($field, $data) && !is_string($data[$field])) $data[$field] = Json::encode($data[$field], [
